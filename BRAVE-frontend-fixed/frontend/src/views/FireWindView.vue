  ...
  <div class="fire-page" :class="{ 'dispatcher-map-mode': isFullMap }">
    ...
    <div class="dashboard-grid" :class="{ 'filters-expanded': showActivityFilters }">
      ...
      <div class="map-card" :class="{ 'full-map-mode': isFullMap }">
        ...
        <div v-if="activePanel" class="map-floating-panel">
          ...
          <div class="floating-search">
            ...
            <input v-model="searchKeyword" type="text" placeholder="Search accidents" />
          </div>
          ...
          <div class="floating-list">
            ...
            >
              ...
              <small>Click to navigate to accident location</small>
            </button>
          </div>
        </div>
        ...
        <div v-if="loading" class="map-loading-overlay">
          <div class="spinner-wrapper">
            ...
            <p>Preparing WebMap, accidents, and analytics...</p>
          </div>
        </div>
        ...
        <!-- Accident stats overlay inside map -->
        <transition name="stats-overlay">
          <div v-if="showAnalyticsPanel" class="map-stats-overlay">
            <div class="map-kpi-stack">
              <div class="map-kpi-card total">
                <FeatherIcon icon="map-pin" />
                <span>Highest Risk District</span>
                <strong>{{ highestRiskDistrict.label }}</strong>
                <small>{{ highestRiskDistrict.count }} accidents</small>
              </div>

              <div class="map-kpi-card warning">
                <FeatherIcon icon="clock" />
                <span>Peak Accident Day</span>
                <strong>{{ peakAccidentDay.label }}</strong>
                <small>{{ peakAccidentDay.count }} accidents</small>
              </div>

              <div class="map-kpi-card red">
                <FeatherIcon icon="alert-triangle" />
                <span>Common Factor</span>
                <strong>{{ commonFactor.label }}</strong>
                <small>{{ commonFactor.count }} cases</small>
              </div>

              <div class="map-kpi-card cyan">
                <FeatherIcon icon="truck" />
                <span>Common Vehicle</span>
                <strong>{{ commonVehicle.label }}</strong>
                <small>{{ commonVehicle.count }} cases</small>
              </div>
            </div>

            <div class="map-chart-row">
              <div class="map-chart-card">
                <h6>Accident by Day</h6>
                <apexchart
                  height="180"
                  type="bar"
                  :options="dayBarOptions"
                  :series="dayBarSeries"
                />
              </div>
              ...
              <div class="map-chart-card">
                ...
                <apexchart
                  height="180"
                  type="bar"
                  :options="factorBarOptions"
                  :series="factorBarSeries"
                />
              </div>
      ...
      <div class="side-panel">
        <div class="activity-feed-card">
          <div class="activity-feed-header">
            <div>
              ...
              <small>Latest accident updates</small>
            </div>
          </div>
        ...
        <div class="accident-kpi-card">
          <div class="kpi-main">
            <div class="kpi-icon" :class="activeKpi.key">
              <div class="kpi-icon-bg"></div>
              <component :is="activeKpi.icon" class="kpi-svg-icon" />
            </div>

            <div class="kpi-content">
              <span>{{ activeKpi.label }}</span>
              <h1 :key="activeKpi.value">{{ activeKpi.value }}</h1>
            </div>
          </div>
          ...
          <div class="kpi-tabs">
            <button
              ...
              :key="item.key"
              :class="{ active: selectedKpi === item.key }"
              @click="selectedKpi = item.key"
            >
              {{ item.label }}
            </button>
          </div>
    ...
    <div class="report-side-grid">
        ...
        <div class="report-analysis-card">
          ...
          <div class="report-chart-area">
            <template v-if="reportView === 'district'">
              <h6>Accidents Count by District</h6>
            ...
            <template v-else>
              <h6>Accidents Count by Severity</h6>
      ...
      <div class="report-analysis-wrapper">
        <div class="analysis-panel">
          <div class="analysis-panel-header">
            <div>
              <h5>Accident Count by Year</h5>
              <span>Highest concentration</span>
            </div>
          </div>

          <apexchart height="320" type="bar" :options="dateBarOptions" :series="dateBarSeries" />
        </div>

        <div class="analysis-panel">
          <div class="analysis-panel-header">
            <div>
              <h5>Top Accident Hotspots</h5>
              <span>{{ hotspotAnalysis.length }} hotspots shown</span>
            </div>
          </div>
          ...
          <apexchart
            height="320"
            type="bar"
            :options="hotspotBarOptions"
            :series="hotspotBarSeries"
          />
        </div>
      </div>
    </div>
...
import { Car, ShieldAlert, Siren, Users } from 'lucide-vue-next'
...
const categoryAnalysis = computed(() => groupCount(allIncidents.value, 'category'))
...
const typeDonutSeries = computed(() => categoryAnalysis.value.map((item) => item.count))
...
const typeDonutOptions = computed(() => ({
  ...
  labels: categoryAnalysis.value.map((item) => item.label),
  colors: chartColors,
  legend: {
    position: 'bottom',
  },
...
const dateAnalysis = computed(() => {
  const grouped = {}

  allIncidents.value.forEach((accident) => {
    const rawDate = Number(accident.rawTime)

    if (!rawDate || Number.isNaN(rawDate)) return
...
const hotspotAnalysis = computed(() => {
  const grouped = {}

  allIncidents.value.forEach((accident) => {
    const label =
      accident.roadName && accident.roadName !== 'Undefined'
        ? accident.roadName
        : accident.mukim || accident.district || 'Unknown'
...
const hotspotBarSeries = computed(() => [
  {
    name: 'Accidents',
    data: hotspotAnalysis.value.map((item) => item.count),
  },
])

const hotspotBarOptions = computed(() => ({
  chart: {
    type: 'bar',
    height: 320,
    toolbar: {
  ...
  xaxis: {
    categories: hotspotAnalysis.value.map((item) => item.label),
    title: {
...
console.log('allIncidents sample:', allIncidents.value?.[0])
console.log('allIncidents length:', allIncidents.value?.length)

const dateBarSeries = computed(() => [
  {
    name: 'Accidents',
    data: dateAnalysis.value.map((item) => item.count),
  },
])

const dateBarOptions = computed(() => ({
  chart: {
    type: 'bar',
    toolbar: { show: false },
  },
  ...
  xaxis: {
    categories: dateAnalysis.value.map((item) => item.label),
    title: { text: 'Year' },
  },
...
function initWeatherOverlay() {
  ...
  if (!MAPTILER_KEY) {
    mapError.value = 'VITE_MAPTILER_API_KEY is not configured.'
    return
  }
...
async function refreshLiveData() {
  ...
  try {
    ...
    const allAccidentFeatures = await getAllFeatures(accidentLayer, '1=1', ['Accident_date DESC'])
...
async function applyDistrictFilter() {
  ...
  const filterLayers = ['Incident Reported', 'Resolved Incident']
...
function mapAccidentFeatures(features) {
  return features.map((feature) => {
    ...
    return {
      ...
      remarks: getAttr(attrs, ['Contributary_factor'], 'Unknown'),
      reportedAt: formatAccidentDate(rawTime),
      rawTime,
      resolved: 'No',

      day: getAttr(attrs, ['Accident_day'], 'Unknown'),
      vehicleType: getAttr(attrs, ['Vehicle_type'], 'Unknown'),
      factor: getAttr(attrs, ['Contributary_factor'], 'Unknown'),
    }
  })
}
...
async function refreshActivityFeedOnly() {
  ...
  try {
    ...
    console.log('ACTIVITY FEED WHERE:', where)

    const accidentFeatures = await getAllFeatures(accidentLayer, where, ['Accident_date DESC'])

    incidents.value = mapAccidentFeatures(accidentFeatures)

    console.log('FILTERED ACTIVITY FEED:', incidents.value)
...
const selectedKpi = ref('accidents')

const totalAccidentCount = computed(() => allIncidents.value.length)

const nonMcgCount = computed(() => {
  return allIncidents.value.filter((item) => item.category === 'Non-MCG').length
})

const fatalityCount = computed(() => {
  return allIncidents.value.filter((item) => String(item.status).toLowerCase().includes('fatal'))
    .length
})

const victimCount = computed(() => {
  return allIncidents.value.reduce((sum, item) => sum + Number(item.victims || 0), 0)
})
...
const kpiItems = computed(() => [
  {
    ...
    value: totalAccidentCount.value,
    icon: Car,
  },
  {
    key: 'nonmcg',
    label: 'Non-MCG',
    value: nonMcgCount.value,
    icon: ShieldAlert,
  },
  {
    key: 'fatalities',
    label: 'Fatalities',
    value: fatalityCount.value,
    icon: Siren,
  },
  {
    key: 'victims',
    label: 'Victims',
    value: victimCount.value,
    icon: Users,
  },
])

const activeKpi = computed(() => {
  return kpiItems.value.find((item) => item.key === selectedKpi.value) || kpiItems.value[0]
})

function topItem(items, fallback = '-') {
  return items[0] || { label: fallback, count: 0 }
}

const dayAnalysis = computed(() => groupCount(allIncidents.value, 'day'))
const factorAnalysis = computed(() => groupCount(allIncidents.value, 'remarks'))
const vehicleAnalysis = computed(() => groupCount(allIncidents.value, 'vehicleType'))

const highestRiskDistrict = computed(() => topItem(districtAnalysis.value))
const peakAccidentDay = computed(() => topItem(dayAnalysis.value))
const commonFactor = computed(() => topItem(factorAnalysis.value))
const commonVehicle = computed(() => topItem(vehicleAnalysis.value))

const dayBarSeries = computed(() => [
  {
    name: 'Accidents',
    data: dayAnalysis.value.map((item) => item.count),
  },
])

const dayBarOptions = computed(() => ({
  chart: {
    type: 'bar',
    toolbar: { show: false },
  ...
  xaxis: {
    categories: dayAnalysis.value.map((item) => item.label),
    labels: {
      style: {
        colors: '#cbd5e1',
        fontSize: '11px',
...
const cleanFactorAnalysis = computed(() => {
  return factorAnalysis.value
    .filter((item) => {
      const label = String(item.label || '').toLowerCase()
      return label && label !== 'undefined' && label !== 'unknown' && label !== '-'
    })
    .slice(0, 6)
})

const factorBarSeries = computed(() => [
  {
    name: 'Cases',
    data: cleanFactorAnalysis.value.map((item) => item.count),
  },
])

const factorBarOptions = computed(() => ({
  chart: {
    type: 'bar',
    toolbar: { show: false },
    background: 'transparent',
  ...
  xaxis: {
    categories: cleanFactorAnalysis.value.map((item) => item.label),
    labels: {
      style: {
        colors: '#cbd5e1',
        fontSize: '11px',
...
const severityDonutSeries = computed(() => severityAnalysis.value.map((item) => item.count))

const severityDonutOptions = computed(() => ({
  chart: { type: 'donut', toolbar: { show: false } },
  labels: severityAnalysis.value.map((item) => item.label),
  ...
  tooltip: { y: { formatter: (value) => `${value} accidents` } },
}))
async function getAllFeatures(layer, where = '1=1', orderByFields = []) {
  if (!layer) return []
...
function buildActivityWhereClause() {
  ...
  if (activityFilters.from) {
    const fromTime = new Date(`${activityFilters.from}T00:00:00`).getTime()
    conditions.push(`Accident_date >= ${fromTime}`)
  }

  if (activityFilters.to) {
    const toTime = new Date(`${activityFilters.to}T23:59:59`).getTime()
    conditions.push(`Accident_date <= ${toTime}`)
  }
...
/* KPI card fills remaining height */
.accident-kpi-card {
  flex: 1;
  display: flex;
  flex-direction: column;
...
.map-kpi-card.total strong {
  color: #fbbf24;
}

.map-kpi-card.warning strong {
  color: #38bdf8;
}

.map-kpi-card.red strong {
  color: #fb7185;
}

.map-kpi-card.cyan strong {
  color: #22d3ee;
}
...
.stats-overlay-enter-active,
.stats-overlay-leave-active {
  transition: all 0.3s ease;
}

.stats-overlay-enter-from,
.stats-overlay-leave-to {
  opacity: 0;
  transform: translateY(18px);
}

.stats-overlay-enter-to,
.stats-overlay-leave-from {
  opacity: 1;
  transform: translateY(0);
}
