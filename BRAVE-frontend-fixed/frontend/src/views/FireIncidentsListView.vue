    ...
    <div class="page-header">
      <div>
        ...
        <p>
          {{
            isOperator
              ? 'Manage all fire incidents pulled from ArcGIS WebMap.'
              : 'View current fire incidents pulled from ArcGIS WebMap.'
          }}
        </p>
      </div>
      ...
      <div class="header-actions">
        ...
        <button
          ...
          type="button"
          @click="$router.push('/fire-list/add')"
        >
        ...
        <table class="table align-middle mb-0">
          <thead>
            <tr>
              ...
              <th>Reported Time</th>
              <th>Remarks</th>
              <th v-if="isOperator" class="text-center">Actions</th>
            </tr>
          </thead>
...
async function addIncident(attributes, geometry) {
  ...
  await http.post('/api/fire-incidents', feature)
}
...
async function updateIncident(attributes, geometry) {
  ...
  await http.put(`/api/fire-incidents/${editingId.value}`, feature)
}
...
async function confirmDelete() {
  ...
  if (!incident || !isOperator.value) return

  deletingId.value = incident.objectId

  try {
    await http.delete(`/api/fire-incidents/${incident.objectId}`)
