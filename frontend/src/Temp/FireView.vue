...
async function handleMapClick(event) {
  ...
  await openIncidentDrawer({ graphic, attrs, layer })
}
...
async function openIncidentPopup({ graphic, attrs, layer }) {
  ...
  view.openPopup({
    ...
    content: `
      <div style="padding:8px 4px;">
        <p><strong>Category :</strong> ${fullAttrs.categories || '-'}</p>
        <p><strong>District :</strong> ${fullAttrs.district || '-'}</p>
        <p><strong>Severity Level :</strong> ${fullAttrs.severity_level || '-'}</p>
        <p><strong>Date & Time :</strong> ${dateValue}</p>
        <p><strong>Incident Details :</strong> ${fullAttrs.more_details || '-'}</p>
      </div>
    `,
  })
}
...
async function refreshLiveData() {
  ...
  try {
    ...
    // ✅ keep backup working sensor logic
    const sensorLayer =
    ...
    const filteredIncidentFeatures = await getFeatures(incidentLayer, {
      limit: 50,
      where: incidentWhere,
    ...
    incidents.value = mapIncidentFeatures(filteredIncidentFeatures)
...
function detectNewIncidents(latestIncidents) {
  const latestIds = new Set(
    latestIncidents.map((incident) => incident.objectId || `${incident.type}-${incident.rawTime}`),
  )
  ...
  const newIncidents = latestIncidents.filter((incident) => {
    const id = incident.objectId || `${incident.type}-${incident.rawTime}`
    return !knownIncidentIds.value.has(id)
  })
  ...
  if (newIncidents.length > 0) {
    newIncidents.forEach((incident) => {
      ...
      notificationStore.addNotification({
        title: `${incident.type || 'Incident'} reported`,
        message: `${incident.location || 'Unknown location'} · ${incident.status || 'Unknown severity'}`,
        severity: severity.includes('critical') || severity.includes('high') ? 'high' : 'medium',
        icon:
          severity.includes('critical') || severity.includes('high') ? 'alert-triangle' : 'info',
        kind: 'incident',
        graphic: incident.graphic,
      })
    })
    ...
    playSpecificAlertSound(hasCritical ? 'siren' : 'notification')
  }
